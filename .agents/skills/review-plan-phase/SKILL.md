...
description: >
  ...
  saves a report to docs/reports/current/, and automatically executes remediation
...
Produce a review report saved to `docs/reports/current/` that:
...
- APIs or behaviors that should be implemented (FastAPI, React 19, etc.)
- Validation, testing, and error-handling expectations
- Architecture decisions committed in writing (Dagster, Istio, Qdrant, Ray Serve, etc.)
...
Verify at minimum:
- Backend: `uv` only. FastAPI schemas via Pydantic v2. `structlog` for logging.
- Frontend: `pnpm` only. React 19, Tailwind 4 styling. Zod boundaries if appropriate.
- Infrastructure: Kind manifests / Dagster / Qdrant helm configurations updated appropriately.
- Plan checkboxes and phase status updated where applicable.
- `docs/README.md` updated when plan status changed.
- Security constraints preserved (Entra ID JWT handling, Istio Ambient policies).
...
- Do tests meaningfully prove the intended behavior (`pytest` for backend)?
...
- `docs/README.md`
- Context READMEs like `docs/context/ARCHITECTURE.md`, `docs/guides/GETTING_STARTED.md`, `docs/guides/AUTHENTICATION.md`
...
1. **Save the report** to `docs/reports/current/<phase>-review-<YYYY-MM-DD>-<hash>.md`
   (e.g., `phase4-review-2026-03-21-k4h7.md`) using the format specified below.
...
2. **Evaluate the remediation path:**
   ...
   - **No `[human]` items exist**: Immediately execute all remediation steps. After
     ...
     ```
     - [x] Remediation complete — see `docs/reports/current/<filename>.md`
     ```
     Then run validations:
     - Backend: `cd backend && uv run ruff check && uv run pytest -q`
     - Frontend: `cd frontend && pnpm test` (or appropriate check if defined in package.json)
...
- [ ] Backend: `cd backend && uv run ruff check && uv run pytest -q`
- [ ] Frontend: Tests and linters pass (`pnpm test` etc.)
...
- Report file is saved to `docs/reports/current/`
...
- Post-remediation validation gates passed (backend pytest/ruff, frontend pnpm tests)
...
- Review and remediate Phase 4 of the infrastructure migration plan.
