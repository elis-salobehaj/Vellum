...
3. Read agent-oriented documentation under `docs/context/` and `docs/guides/`:
   - [ARCHITECTURE.md](../../../docs/context/ARCHITECTURE.md) — current runtime surface,
     async ingestion (Dagster), vector DB (Qdrant), Istio Ambient mesh, Backend/Frontend structures.
   - [WORKFLOWS.md](../../../docs/context/WORKFLOWS.md) — operational workflows, documentation practices.
   - [GETTING_STARTED.md](../../../docs/guides/GETTING_STARTED.md) — development configurations and setup scripts.

4. Read relevant design documents if the feature touches areas with existing design decisions:
   - `docs/designs/` for design summaries (e.g., ADRs)
...
7. If the feature introduces dependencies on external systems, APIs, or libraries, research
   their compatibility with the Vellum stack (`uv`-managed Python 3.12+ backend with FastAPI/Pydantic, 
   or `pnpm`-managed Node 24 frontend with React/Vite/Tailwind).

> **Token budget awareness**: Leverage provided documentation first. Only dig into raw source files when the documentation lacks the needed detail for accurate planning.
...
3. Identify constraints from AGENTS.md and existing conventions:
   - Backend tooling must use `uv`. FastAPI models must use `Pydantic v2`.
   - Frontend tooling must use `pnpm`, `React 19`, and `Tailwind CSS 4`.
   - Ingestion tasks use Dagster concepts (`@asset`, `@sensor`) not raw scripts.
   - Istio Ambient handles zero-trust L4 networking and L7 auth; no sidecars.
   - Plan and docs update requirements must be met.
...
1. Identify the decision points. For each:
   ...
   - Evaluate each against Vellum's stack constraints (Python 3.12 compatibility, Pydantic integration,
     React Server/Client boundaries, maintenance health, license)
...
- Update all affected docs under `docs/context/` (ARCHITECTURE.md, WORKFLOWS.md) to reflect the changes
...
- Run `cd backend && uv run pytest` or `cd frontend && pnpm test` to confirm the full suite passes after the phase
...
- When the plan is intricate and a phase produces intermediate scaffolding that cannot
  ...
  completes the testable surface. The deferral must be explicit in the plan.
...
2. Update [docs/README.md](../../../docs/README.md):
   - Add the new plan to the **Active Plans** section under the matching
     category (**Backlog** or **Active**) based on where the file was saved
   - Add a row to the table with status ⏳ Backlog or 🚧 Active and a brief summary
...
- Obey all critical rules from AGENTS.md — `uv`, `pnpm`, Backend/Frontend constraints,
  Istio/Dagster components.
- Do not recommend tools, libraries, or patterns without verifying Python/FastAPI or React/Vite compatibility.

## Completion Checks

Before presenting the plan as ready:
- The plan file exists on disk at `docs/plans/backlog/<name>.md` (or `active/`)
...
- Every phase includes a test coverage step, or explicitly justifies deferral
...
- When architectural changes are involved, the Mermaid diagram appears at the top

## Preferred Prompts

- Plan the implementation of [feature] for Vellum.
- Create an implementation plan for adding [capability] to the RAG architecture.
- I want to add [library/tool] to Vellum — plan the integration.
