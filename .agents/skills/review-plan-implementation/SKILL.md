...
- **System boundaries**: Are module boundaries explicit? Does the plan introduce ambiguous
  ...
  cohesive surfaces (e.g., mixing FastAPI backend responsibilities with Dagster async jobs)?
...
- **Synchronous bottlenecks**: Does the plan introduce synchronous operations in
  ...
  appropriate async/batch patterns? Will any phase block the event loop?
- **Data flow completeness**: Trace data from entry (API, UI upload) to exit (Qdrant index,
  Chat response, Dagster log). Are there dead-end paths where data is produced but never
  consumed? Are there paths where errors are silently swallowed?
- **Integration seams**: Where the plan connects to existing systems (Istio, Entra ID,
  Ray Serve, TEI, Qdrant), are the contracts explicit? Are there implicit assumptions
  about the shape of data crossing boundaries?
...
- **Runtime nativity**: Are proposed libraries native to the target runtime (`uv`-managed Python 3.12+ or `pnpm`-managed Node 24)? Are obsolete or incompatible packages proposed?
...
- **Maintenance health**: Is each proposed library actively maintained? To evaluate:
  1. **Prefer live data**: Use HTTP tools (e.g., `fetch_webpage`) to check the library's
     GitHub repository (stars, last commit date, open/closed issue ratio)
     and PyPI/NPM registry page.
  2. **Fallback**: Use training data to assess maintenance health, but state when you are
...
- **Drop-in upgrades**: If the plan retains an existing dependency, is there a clearly
  superior alternative that is more performant, more secure, and close to a drop-in replacement?
...
- **Execution sandboxing**: Does the plan introduce code execution paths (subprocess,
  ...
  operations constrained to validated paths with prefix checks?
- **Input validation boundaries**: Are all new external data entry points (API endpoints,
  environment variables, LLM outputs, UI form inputs) validated with Pydantic v2 schemas 
  (Backend) or Zod (Frontend)? Are there paths where external data crosses a trust boundary
  without validation?
- **Authentication and authorization**: Does the plan handle credentials correctly?
  Are secrets loaded from environment only, never hardcoded or logged? Does the plan
  interact securely with the Entra ID / Istio Ambient L7 JWT validations?
- **Blast radius of failure**: If the new feature fails catastrophically, what else
  breaks? Can a failure in one ingestion pipeline corrupt another? Can malformed input
  crash the server or exhaust resources? Is failure isolated to the request?
- **Dependency audit**: If the plan introduces dependencies, verify if they have known vulnerabilities.
- **SSRF and path traversal**: Does the plan accept URLs, file paths, or references from
  external input? Are they validated and restricted to expected domains and paths?
...
- **Failure modes**: For each external call (LLM API, Qdrant DB, file system, Istio mesh),
  what happens on: timeout, 429 rate limit, 500 server error, malformed response,
  empty response, network partition? Are retries bounded? Is there backoff?
- **LLM output handling**: Does the plan account for malformed, partial, or
  hallucinated LLM structured outputs? Is validation applied to LLM responses
  before consumption?
- **Concurrency and locking**: If the plan introduces concurrent operations on shared
  resources (document storage, vector DB), is locking or serialization addressed?
- **Idempotency**: Can the same event be safely processed twice? Does the plan address
  duplicate document uploads, retry storms, or at-least-once delivery semantics?
- **Resource exhaustion**: Does the plan bound memory usage, subprocess count, disk
  consumption, or open connections? Can a malicious or malformed payload with thousands of
  pages exhaust system resources?
- **Rollback strategy**: If a phase is deployed and fails, how is the change rolled back?
- **Graceful degradation**: When the plan introduces a new capability that depends on
  an external service (LLM API, Ray Serve), does the system degrade gracefully when
  that service is unavailable, or does it hard-fail?
...
- **AGENTS.md compliance**: Does the plan respect all critical rules from AGENTS.md?
  `uv` for backend, `pnpm` for frontend, Pydantic validations, `ruff` linting, security
  sandboxing, and plan/docs update requirements.
