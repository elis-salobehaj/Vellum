...
def submit_run(
    ...
    max_docs=2,
    top_k=2,
    splitter_mode="semantic",
    model_name="BAAI/bge-small-en-v1.5",
    enable_cache=False,
    embeddings_service_url="http://embeddings-service.kubeflow-vellum/v1",
    cleanup=False,
):
    ...
    print(
        f"🔗 View run: http://localhost:8080/_/pipeline/?ns=kubeflow-vellum#/runs/details/{run.run_id}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--enable_cache", action="store_true", help="Enable KFP caching"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--cleanup", action="store_true", help="Cleanup collection before ingestion"
    )
